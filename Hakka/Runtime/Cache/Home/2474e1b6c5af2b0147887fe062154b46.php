...
<div class="title">
    <ol>
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><li><a  href="<?php echo U('Home/Corr/gywm',array('clid'=>$list['id']));?>" <?php if(($clid) == $list['id']): ?>class="title-active load"<?php endif; ?>><?php echo ($list["classname"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ol>
</div>
<ul class="example-list curl new-list fly">
        <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><li>
            <a class="load" href="/Home/Corr/gy/id/<?php echo ($list["id"]); ?>">
                <h3><?php echo ($list["title"]); ?></h3>
                <p><?php echo ($list["title"]); ?> <?php echo date("Y-m-d H:i",$list['addtime']);?></p>
...
<script>
    ...
    function ajaxloadMore() {
        var data = { Page: page, clid: '<?php echo ($clid); ?>' };
        $.ajax({
            type: 'POST',
            url: '/Home/Corr/lists',
            data: data,
